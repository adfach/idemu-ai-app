  ...
  return (
    <Card className="relative w-full shadow-2xl">
      <CardHeader className="text-center">
        <CardTitle className="font-headline text-2xl">{t('auth.welcome_back')}</CardTitle>
        <CardDescription>{t('auth.login_prompt')}</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Button variant="outline" onClick={() => signIn('google')} disabled={loading}><GoogleIcon /> Google</Button>
          <Button variant="outline" onClick={() => signIn('github')} disabled={loading}><GithubIcon /> GitHub</Button>
        </div>
      </CardContent>
      <CardFooter className="flex justify-center text-sm">
        <p className="text-muted-foreground">
          {t('auth.signup_redirect')}{' '}
